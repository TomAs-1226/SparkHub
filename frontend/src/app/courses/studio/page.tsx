...
import { BookOpenCheck, CalendarDays, ClipboardList, Copy, Link2, ShieldCheck, UploadCloud, UsersRound, Video, Layers, Settings2, LayoutDashboard } from "lucide-react";
...
type StudioTab = "Overview" | "Courses" | "Workspace" | "Settings";
const TABS: StudioTab[] = ["Overview", "Courses", "Workspace", "Settings"];
            ...
            <main className="mx-auto w-full max-w-6xl px-4 pb-16 pt-10">
                ...
                >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        <div>
                            <p className="text-xs font-semibold uppercase tracking-wide text-[#2D8F80]">Creator studio</p>
                    ...
                    {/* Tab pills */}
                    <div className="mt-8 flex gap-1 rounded-full bg-slate-100 p-1 w-fit">
                        {TABS.map((tab) => (
                            <button
                                key={tab}
                        ...
                        <AnimatePresence mode="wait">
                            {activeTab === "Overview" && (
                                ...
                                >
                                    ...
                                    <div className="grid gap-4 sm:grid-cols-3">
                                        <div className="rounded-[24px] border border-white/60 bg-gradient-to-br from-[#EEF0FF] to-white p-5">
                                            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-[#2B2E83]">
                                                <LayoutDashboard className="h-4 w-4" />
                                                Total courses
                                            </div>
                                            <p className="mt-3 text-4xl font-bold text-[#2B2E83]">{courses.length}</p>
                                        </div>
                                        <div className="rounded-[24px] border border-white/60 bg-gradient-to-br from-[#E6F7F5] to-white p-5">
                                            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-[#2D8F80]">
                                                <BookOpenCheck className="h-4 w-4" />
                                                Published
                                            </div>
                                            <p className="mt-3 text-4xl font-bold text-[#2D8F80]">{publishedCount}</p>
                                        </div>
                                        <div className="rounded-[24px] border border-slate-100 bg-slate-50 p-5">
                                            ...
                                            <p className="mt-3 text-4xl font-bold text-slate-600">{draftCount}</p>
                                        </div>
                                    </div>
                                    ...
                                    {detail ? (
                                        <div className="rounded-[24px] border border-slate-100 bg-white p-6">
                                            <div className="flex flex-wrap items-start justify-between gap-4">
                                                <div>
                                                    <span className={`inline-block rounded-full px-3 py-0.5 text-xs font-semibold ${detail.isPublished ? "bg-[#E6F7F5] text-[#2D8F80]" : "bg-slate-100 text-slate-500"}`}>
                                                        {detail.isPublished ? "Published" : "Draft"}
                                                    </span>
                                                    <h2 className="mt-2 text-2xl font-semibold text-slate-900">{detail.title}</h2>
                                                    <p className="mt-1 text-sm text-slate-500">{detail.summary || "No summary provided yet."}</p>
                                                    {detail.tags && detail.tags.length > 0 && (
                                                        <div className="mt-3 flex flex-wrap gap-1.5">
                                                            {detail.tags.map((tag) => (
                                                                <span key={tag.slug} className="rounded-full bg-[#EEF0FF] px-2.5 py-0.5 text-xs font-semibold text-[#2B2E83]">
                                                                    {tag.label}
                                                                </span>
                                                            ))}
                                                        </div>
                                            ...
                                            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                                                <div className="rounded-2xl bg-slate-50 px-3 py-2 text-center">
                                                    ...
                                                    <p className="text-lg font-bold text-slate-700">{detail.sessions.length}</p>
                                                </div>
                                                <div className="rounded-2xl bg-slate-50 px-3 py-2 text-center">
                                                    <p className="text-xs text-slate-500">Assignments</p>
                                                    <p className="text-lg font-bold text-slate-700">{detail.assignments.length}</p>
                                                </div>
                                                <div className="rounded-2xl bg-slate-50 px-3 py-2 text-center">
                                                    <p className="text-xs text-slate-500">Lessons</p>
                                                    <p className="text-lg font-bold text-slate-700">{detail.lessons.length}</p>
                                                </div>
                                                <div className="rounded-2xl bg-slate-50 px-3 py-2 text-center">
                                                    <p className="text-xs text-slate-500">Enrollments</p>
                                                    <p className="text-lg font-bold text-slate-700">{enrollments.length}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="rounded-[24px] border border-dashed border-slate-200 bg-white/70 p-6 text-sm text-slate-500">
                                            Select or create a course in the <button onClick={() => setActiveTab("Courses")} className="font-semibold text-[#2D8F80] underline-offset-2 hover:underline">Courses tab</button> to see its overview here.
                                        </div>
                                    )}
                                    ...
                                    {courses.length > 0 && (
                                        <div className="rounded-[24px] border border-slate-100 bg-white p-5">
                                            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">All your courses</p>
                                                ...
                                                {courses.map((course) => (
                                                    ...
                                                    >
                                                        <span className="flex items-center justify-between">
                                                            {course.title}
                                                            <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${course.isPublished ? "bg-[#E6F7F5] text-[#2D8F80]" : "bg-slate-100 text-slate-500"}`}>
                            ...
                            {activeTab === "Courses" && (
                                ...
                                >
                                    <div className="grid gap-6 lg:grid-cols-[280px,1fr]">
                                        {/* Sidebar: course list + create form */}
                                        ...
                                        {/* Main: course detail + management panels */}
                            ...
                            {activeTab === "Workspace" && (
                                ...
                                >
                                    {detail ? (
                                        <>
                                            <div className="rounded-[24px] border border-[#E6F7F5] bg-[#F2FAFB] px-5 py-3">
                                                <p className="text-xs font-semibold text-[#2D8F80]">Working on: <span className="text-slate-700">{detail.title}</span></p>
                                            </div>

                                            {/* Lesson decks + Meeting links */}
                                            <div className="grid gap-6 lg:grid-cols-2">
                                        ...
                                        <div className="rounded-3xl border border-dashed border-slate-200 bg-white/70 p-5 text-sm text-slate-500">
                                            Select a course in the <button onClick={() => setActiveTab("Courses")} className="font-semibold text-[#2D8F80] underline-offset-2 hover:underline">Courses tab</button> to use the workspace.
                                        </div>
                                    )}
                                </motion.div>
                            )}
                            ...
                            {activeTab === "Settings" && (
                                ...
                                >
                                    <div className="rounded-[24px] border border-slate-100 bg-white p-6">
                                        <div className="flex items-center gap-2 text-sm font-semibold text-slate-700">
                                            <Settings2 className="h-5 w-5 text-[#2D8F80]" />
                                            Studio settings
                                        </div>
                                        <p className="mt-2 text-xs text-slate-500">
                                            Configure your creator studio preferences. More settings coming soon.
                                        </p>
                                        ...
                                        {detail ? (
                                            <div className="mt-6 space-y-4">
                                                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                                                    ...
                                                    <p className="mt-1 text-base font-semibold text-slate-800">{detail.title}</p>
                                                    <div className="mt-3 flex flex-wrap gap-3 text-xs">
                                                        <div className="rounded-xl bg-white px-3 py-2 shadow-sm">
                                                            <span className="text-slate-500">Status: </span>
                                                            <span className={`font-semibold ${detail.isPublished ? "text-[#2D8F80]" : "text-slate-500"}`}>
                                                                {detail.isPublished ? "Published" : "Draft"}
                                                            </span>
                                                        </div>
                                                        <div className="rounded-xl bg-white px-3 py-2 shadow-sm">
                                                            <span className="text-slate-500">Join code: </span>
                                                            <span className="font-bold tracking-widest text-[#2B2E83]">{detail.joinCode || "------"}</span>
                                                        </div>
                                                    </div>
                                                    <div className="mt-4 flex gap-2">
                                                        <button
                                                            type="button"
                                                            onClick={copyJoinCode}
                                                            className="flex items-center gap-1.5 rounded-full border border-slate-200 px-4 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-100"
                                                        >
                                                        ...
                                                        <button
                                                            type="button"
                                                            onClick={handleRegenerateJoinCode}
                                                            className="rounded-full border border-[#CFE3E0] px-4 py-1.5 text-xs font-semibold text-[#2D8F80] hover:bg-[#F2FAFB]"
                                                        >
                                                            Regenerate code
                                                        </button>
                                                    </div>
                                            ...
                                            <div className="mt-6 rounded-2xl border border-dashed border-slate-200 bg-white/70 p-4 text-sm text-slate-500">
                                                Select a course in the <button onClick={() => setActiveTab("Courses")} className="font-semibold text-[#2D8F80] underline-offset-2 hover:underline">Courses tab</button> to see its settings.
                                            </div>
                                        )}
                                    </div>

                                    <div className="rounded-[24px] border border-slate-100 bg-white p-6">
                                        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Your account role</p>
                                        <p className="mt-2 text-sm text-slate-700">
                                            Signed in as <span className="font-semibold">{user?.name || user?.email}</span> with role{" "}
                                            <span className="rounded-full bg-[#EEF0FF] px-2 py-0.5 text-xs font-bold text-[#2B2E83]">{user?.role}</span>
                                        </p>
                                    </div>
                                </motion.div>
                            )}
